...
\textbf{Tentative g}
\begin{enumerate}
    \item $$g=1-a_i+\omega\int_{0}^{1}a_j\mathrm{d}j$$ %Not consistent with what you wrote in (44)
    \item $$g=\omega\left(1-\int_{0}^{1}a_j\mathrm{d}j\right)$$ %Problematic because the return is zero when omega=0.
    \item $$g=1-\omega\int_{0}^{1}a_j\mathrm{d}j$$ %The best I could come up with
\end{enumerate}
